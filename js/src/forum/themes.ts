...
export type ThemeContent = [string, string | null, string | null];

/**
 * Returns the [idle, success, failure] button innerHTML for a theme.
 */
export default function getThemeContent(theme: string): ThemeContent {
  const copyLabel = extractText(app.translator.trans('linkrobins-clipboard.forum.action_copy'));
  const okLabel = extractText(app.translator.trans('linkrobins-clipboard.forum.ok_btn'));
  const errorLabel = extractText(app.translator.trans('linkrobins-clipboard.forum.error_btn'));
  ...
  switch (theme) {
    ...
    case 'segmentfault':
      return ['<i class="fas fa-copy"></i>', '<i class="fas fa-check"></i>', '<i class="fas fa-times"></i>'];
    default:
      return [
